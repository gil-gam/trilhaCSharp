...
namespace WebSupplier.Domain.Services
{
    public class ProductService : ServiceBase<Product>, IProductService
    {
        ...
        public override async Task<Product> FindById(Guid id)
        {
            ...
            if (result == null) Notify("Product not located.");
        ...
        public async Task Insert(Product product)
        {
            ...
            if (result != null)
            {
                if (result.Name == product.Name)
                    Notify($"The Name {product.Name} already registered for another product.");
                if (result.BarCode == product.BarCode)
                    Notify($"The BarCode {product.BarCode} already registered for another product.");
